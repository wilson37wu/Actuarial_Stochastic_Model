...
class PolicyDataGenerator:
    ...
    def generate_term_policy(self) -> TermPolicy:
    ...
    def generate_whole_life_policy(self, participating: bool = False) -> Union[WholeLifePolicy, ParticipatingPolicy]:
        ...
        base_args = {
            ...
            'smoking_status': random.choice(list(SmokingStatus)),
            'occupation_class': random.choice(list(OccupationClass)),
            'dividend_option': random.choice(list(DividendOption)),
            'investment_strategy': random.choice(list(InvestmentStrategy))
        }
        
        if participating:
            return ParticipatingPolicy(
                **base_args,
                bonus_rate=random.uniform(0.02, 0.05)
            )
        else:
            return WholeLifePolicy(**base_args)
    
    def generate_policy_mix(self, 
                          num_policies: int,
                          term_ratio: float = 0.4,
                          whole_life_ratio: float = 0.3,
                          participating_ratio: float = 0.3) -> List[Union[TermPolicy, WholeLifePolicy, ParticipatingPolicy]]:
        ...
        for _ in range(num_term):
            policy = self.generate_term_policy()
            policies.append(policy)
        
        # Generate whole life insurance policies
        for _ in range(num_whole_life):
            policy = self.generate_whole_life_policy(participating=False)
            policies.append(policy)
        
        # Generate participating whole life policies
        for _ in range(num_participating):
            policy = self.generate_whole_life_policy(participating=True)
            policies.append(policy)
