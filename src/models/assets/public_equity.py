...
class EquityModel:
    ...
    This model handles equity price projections and cash flow calculations
    using parameters defined in the ModelConfig.
    """
    
    def __init__(self, config: ModelConfig):
        """Initialize equity model with configuration.
        
        Args:
            config: ModelConfig instance containing equity parameters
        """
        self.config = config
        equity_params = config.asset_params.get('equity', {})
        self.market_volatility = equity_params.get('market_volatility', 0.15)
        self.sector_correlations = self._load_sector_correlations()
        
    def _load_sector_correlations(self) -> pd.DataFrame:
        """Load sector correlation matrix from configuration."""
        equity_params = self.config.asset_params.get('equity', {})
        sector_weights = equity_params.get('sector_weights', {})
        sectors = list(sector_weights.keys())
        
        # For now, use identity matrix if correlations not specified
    ...
    def project_returns(self, n_periods: int = 12) -> pd.Series:
        ...
        """
        equity_params = self.config.asset_params.get('equity', {})
        expected_return = equity_params.get('expected_return', 0.08)
        volatility = equity_params.get('market_volatility', 0.15)
    ...
    def update_parameters(self, config: ModelConfig) -> None:
