"""
Asset Liability Management (ALM) Utility Functions

This module provides utility functions for ALM calculations, risk metrics,
and portfolio analysis. It includes implementations of key financial mathematics
concepts used in asset-liability management.
...
class RiskMetrics:
    """
    Container for various risk metrics used in ALM analysis.
    
    Attributes:
        var_95 (float): Value at Risk at 95% confidence level
        cvar_95 (float): Conditional Value at Risk at 95% confidence level
        duration (float): Portfolio duration
        convexity (float): Portfolio convexity
    """
    var_95: float
    cvar_95: float
    duration: float
...
def calculate_present_value(
    ...
    Example:
        >>> calculate_present_value([100, 200], [1, 2], [0.05, 0.05])
        285.94...
    """
    return sum(
        cf / (1 + r) ** t
...
def calculate_duration(
    ...
    Example:
        >>> calculate_duration([100, 200], [1, 2], 0.05)
        1.65...
    """
    pv = calculate_present_value(cash_flows, times, [discount_rate] * len(times))
    weighted_times = sum(
...
def calculate_convexity(
    ...
    Example:
        >>> calculate_convexity([100, 200], [1, 2], 0.05)
        3.89...
    """
    pv = calculate_present_value(cash_flows, times, [discount_rate] * len(times))
